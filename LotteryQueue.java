...
public class LotteryQueue {
    ...
    static int numParticipant = 11223;
    ...
    public static PriorityQueue<LotteryParticipant> compileLotteryParticipantQueue() {
        ...
        for (int i = 0; i < 10; i++) {
            LotteryParticipant participant = new LotteryParticipant(numParticipant++,
                    LotteryParticipant.generateRandomName(), LotteryParticipant.generateRandomSurname());
    ...
    public static void deliverLotteryPrises(PriorityQueue<LotteryParticipant> participantsQueue,
            ArrayList<Toy> toysForLottery) {
        if (participantsQueue.size() == toysForLottery.size()) {
            ...
            for (index = 0; index < toysForLottery.size(); index++) {
                WriteToFile.writeDataToFile(
                        participantsQueue.peek() + "получает приз " + toysForLottery.get(index) + "\n");
            ...
            throw new RuntimeException("Несовпадение кол-ва участников и призов, лотерею придется отложить");
    }
}
